...
function trainPosition(d,fromIndex,toIndex){const stops=Array.isArray(d?.fermate)?d.fermate:[],lastName=String(d?.stazioneUltimoRilevamento||'').trim();let detected=lastName?stops.findIndex(stop=>norm(stop?.stazione)===norm(lastName)):-1;if(detected<0){for(let i=fromIndex;i<=toIndex;i++){const stop=stops[i];if(stop?.partenzaReale||stop?.arrivoReale||stop?.effettiva)detected=i}}if(detected<0)return null;const bounded=Math.max(fromIndex,Math.min(toIndex,detected)),current=String(stops[bounded]?.stazione||lastName||'').trim(),next=bounded<toIndex?String(stops[bounded+1]?.stazione||'').trim():null,progress=toIndex===fromIndex?1:(bounded-fromIndex)/(toIndex-fromIndex);return{currentStation:current||null,nextStation:next||null,progress:Number(progress.toFixed(3)),checkedAt:millisToTime(d?.oraUltimoRilevamento)||null}}
